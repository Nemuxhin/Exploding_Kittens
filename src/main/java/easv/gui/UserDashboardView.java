...
public class UserDashboardView {
    ...
    public Parent create() {
        ...
        UserPortalModel.ScanProfile[] selectedProfile = {null};
        String[] currentBoxId = {""};
        List<UserPortalModel.ScanningDocument>[] scanningDocuments = new List[]{copyScanningDocuments()};
        List<UserPortalModel.ScanningDocument>[] scannedWorkspaceDocuments = new List[]{new ArrayList<>()};
        UserPortalModel.ScanningDocument[] activeScanningDocument = {null};
        UserPortalModel.ScanningFile[] activeScanningFile = {null};
        ...
        render[0] = () -> {
            ...
            Consumer<UserPortalModel.ScanProfile> onSelectProfile = profile -> {
            ...
            if (currentPage[0] == PageState.SCAN_PROGRESS || currentPage[0] == PageState.SCAN_COMPLETE) {
                ...
                root.setLeft(buildSidebar(currentPage[0], darkMode[0], onNavigate, () -> {
            ...
            root.setCenter(buildContent(
                    ...
                    boxId -> {
                        ...
                        scanningDocuments[0] = copyScanningDocuments();
                        ...
                        scanProgressTimeline[0] = new Timeline(new KeyFrame(Duration.millis(35), event -> {
                            ...
                            if (scanProgress[0] >= 100) {
                                for (UserPortalModel.ScanningDocument document : scanningDocuments[0]) {
                    ...
                    offset -> {
                        List<UserPortalModel.ScanningFile> qaFiles = flattenScannedFiles(scannedWorkspaceDocuments[0]);
    ...
    private VBox buildSidebar(PageState currentPage,
                              boolean darkMode,
        ...
        Label logoBadge = new Label("WL");
        logoBadge.getStyleClass().add("brand-mark");
        VBox brandText = new VBox(2);
        Label brandTitle = new Label("WebLager");
        brandTitle.getStyleClass().add("brand-title");
        Label brandSubtitle = new Label("Scanning Portal");
        brandSubtitle.getStyleClass().add("brand-subtitle");
        brandText.getChildren().addAll(brandTitle, brandSubtitle);
        brandRow.getChildren().addAll(logoBadge, brandText);

        VBox navigation = new VBox(8);
        for (UserPortalModel.NavigationItem item : portalModel.sidebarItems()) {
            navigation.getChildren().add(buildNavItem(item, currentPage, onNavigate));
        }
    ...
    private HBox buildNavItem(UserPortalModel.NavigationItem item, PageState currentPage, Consumer<PageState> onNavigate) {
        PageState page = mapPage(item.label());
    ...
    private BorderPane buildContent(PageState pageState,
                                    UserPortalModel.ScanProfile selectedProfile,
                                    String currentBoxId,
        ...
        Parent pageBody = switch (pageState) {
            ...
            case PROFILES -> buildProfilesBody(selectedProfile, currentBoxId, onSelectProfile, onStartScanning);
            case LOGS -> buildLogsBody();
            case MY_SCANS -> buildMyScansBody();
            case SCAN_PROGRESS -> buildScanProgressBody(selectedProfile, currentBoxId, scanProgress, onBackToProfiles);
            case SCAN_COMPLETE -> buildScanCompleteBody(selectedProfile, currentBoxId, onScanningChanged, onBackToProfiles);
    ...
    private Parent buildProfilesBody(UserPortalModel.ScanProfile selectedProfile,
                                     String currentBoxId,
                                     Consumer<UserPortalModel.ScanProfile> onSelectProfile,
                                     Consumer<String> onStartScanning) {
    ...
    private VBox buildScanProgressBody(UserPortalModel.ScanProfile selectedProfile,
                                       String currentBoxId,
                                       double scanProgress,
        ...
        details.add(scanInfoLabel("Started by:"), 0, 2);
        details.add(scanInfoValue("John Doe"), 1, 2);
    ...
    private VBox buildScanCompleteBody(UserPortalModel.ScanProfile selectedProfile,
                                       String currentBoxId,
        ...
        summary.add(infoBlock("Pages Scanned", "156 pages"), 0, 2);
        summary.add(infoBlock("Time Completed", "2:52 PM"), 1, 2);
    ...
    private String scanProfileName(UserPortalModel.ScanProfile selectedProfile) {
        return selectedProfile == null ? portalModel.activeProfile().profileName() : selectedProfile.title();
    }

    private VBox buildScanningBody(UserPortalModel.ScanProfile selectedProfile,
                                   String currentBoxId,
                                   List<UserPortalModel.ScanningDocument> scanningDocuments,
                                   List<UserPortalModel.ScanningDocument> scannedWorkspaceDocuments,
                                   UserPortalModel.ScanningDocument activeScanningDocument,
    ...
    private Parent buildQaReviewBody(UserPortalModel.ScanProfile selectedProfile,
                                     String currentBoxId,
                                     List<UserPortalModel.ScanningDocument> scannedWorkspaceDocuments,
                                     UserPortalModel.ScanningDocument activeScanningDocument,
                                     UserPortalModel.ScanningFile activeScanningFile,
    ...
    private VBox buildQaFileRow(UserPortalModel.ScanningFile file,
                                boolean selected,
                                Consumer<UserPortalModel.ScanningFile> onSelectQaFile) {
    ...
    private VBox scanningDocumentItem(UserPortalModel.ScanningDocument document,
                                      boolean selected,
                                      Consumer<UserPortalModel.ScanningDocument> onSelectScanningDocument) {
    ...
    private String documentQaStatus(UserPortalModel.ScanningDocument document,
                                    UserPortalModel.ScanningDocument activeDocument,
                                    Set<String> qaCompletedDocuments) {
    ...
    private VBox buildFilePagesBody(UserPortalModel.ScanProfile selectedProfile,
                                    String currentBoxId,
                                    UserPortalModel.ScanningDocument activeScanningDocument,
                                    UserPortalModel.ScanningFile activeScanningFile,
                                    Runnable onBackToScanning) {
    ...
    private StackPane buildActionIconBadge(UserPortalModel.ActionCard card) {
        StackPane badge = new StackPane();
    ...
    private HBox exportActionCell(UserPortalModel.ExportRow export) {
    ...
    private UserPortalModel.ScanningDocument findScanningDocument(List<UserPortalModel.ScanningDocument> scanningDocuments, String title) {
    ...
    private UserPortalModel.ScanningDocument findDocumentForFile(List<UserPortalModel.ScanningDocument> documents,
                                                                 UserPortalModel.ScanningFile file) {
        if (file == null) {
            return null;
        }
    ...
    private List<UserPortalModel.ScanningFile> flattenScannedFiles(List<UserPortalModel.ScanningDocument> documents) {
    ...
    private boolean moveOrReorderFile(UserPortalModel.ScanningDocument sourceDocument,
    ...
    private int totalFiles(List<UserPortalModel.ScanningDocument> documents) {
        int total = 0;
        for (UserPortalModel.ScanningDocument document : documents) {
